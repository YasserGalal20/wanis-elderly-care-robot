...
class ExplorerNode(Node):
    ...
    def map_callback(self, msg):
        ...
        # Store the initial position only once after the first map is received
        if not self.origin_set:
            self.origin_x  = msg.info.origin.position.x
            self.origin_y = msg.info.origin.position.y
            self.get_logger().info(f"Initial position set to: x={self.origin_x }, y={self.origin_y }")
    ...
    def choose_frontier(self, frontiers):
        ...
        if chosen_frontier:
            ...
            self.checked +=1
            if self.checked >= 5:
                self.get_logger().warning("Checked many times and didn't find")
                self.get_logger().info("Going back to initail postition, saving map, and closing node")
                goal_x =   self.origin_x * 0
                goal_y =   self.origin_y * 0
